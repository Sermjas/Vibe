...
def _sync_generate_raw_text(image_bytes: bytes, api_key: str) -> str | None:
    ...
    for attempt in range(_GEMINI_RETRY_MAX_ATTEMPTS):
        try:
            ...
            if e.code == 404:
                logger.error(
                    "Gemini вернул 404 (модель/ресурс недоступен): %s",
                    e.message or e,
                )
                return None
            ...
            if e.code in (500, 502, 503) and attempt < _GEMINI_RETRY_MAX_ATTEMPTS - 1:
                wait = _GEMINI_RETRY_BASE_SEC * (2**attempt)
                logger.warning(
                    f"Временная ошибка сервера Gemini (HTTP {e.code}), попытка {attempt + 1}/{_GEMINI_RETRY_MAX_ATTEMPTS}, "
                    f"пауза {wait:.1f} с."
                )
                time.sleep(wait)
                continue
            logger.error(
                "Ошибка сервера Gemini API: HTTP %s. %s",
                e.code,
                e.message or e,
            )
            return None
